...
# The characters allowed in an HTTP reason phrase, from RFC 9112:
# reason-phrase = *( HTAB / SP / VCHAR / obs-text )
# The pattern is anchored with \Z (and used with match instead of fullmatch)
# so that it behaves the same on all supported versions of python.
_reason_phrase_re = re.compile(r"[\t \x21-\x7e\x80-\xff]*\Z")
...
class RequestHandler(object):
    ...
    def set_status(self, status_code, reason=None):
        ...
        :arg str reason: Human-readable reason phrase describing the status
            ...
            this argument should only be used if you need to use a
        ...
        if reason is not None:
            ...
            if "<" in reason or not _reason_phrase_re.match(reason):
